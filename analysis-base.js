...
const FDV_SCENARIOS = Array.from({ length: 18 }, (_, index) => {
  ...
  return {
    fdv,
    name: fdv === BASE_FDV_USD ? "Base" : formatScenarioFdv(fdv),
  };
});
...
function getProjection() {
  ...
  const baseScenario = scenarios.find((scenario) => scenario.name === "Base") ?? scenarios[0];
...
function renderTable() {
  if (state.wallets.length === 0) {
    elements.farmRows.innerHTML = '<tr class="empty-row"><td colspan="7">Save wallet addresses above to load data.</td></tr>';
    return;
  }
  ...
  elements.farmRows.innerHTML = state.rows
    .map((row, index) => {
      ...
      const status = row.ok ? (row.points > 0 ? "Tracking" : "No points") : "Error";
      return `
        <tr>
          <td>
          ...
          <td>${formatPoints.format(row.pending)}</td>
...
function renderFdvScenarios(projection) {
  elements.fdvScenarioSelect.innerHTML = projection.scenarios
    .map((scenario, index) => {
      ...
      return `<option value="${index}">${scenario.name} - ${formatUsdCompact.format(scenario.fdv)} FDV - ${formatUsd.format(scenario.estimatedValue)} (${basis})</option>`;
    })
    .join("");
    ...
    .map((scenario, index) => {
      ...
      return `
        <tr tabindex="0" data-scenario-index="${index}" aria-label="${scenario.name} FDV scenario estimated value ${formatUsd.format(scenario.estimatedValue)}">
          ...
          <td>${basis}</td>
          <td>${formatUsdCompact.format(scenario.fdv)}</td>
          <td>${formatUsdCompact.format(scenario.airdropPool)}</td>
  ...
  const baseIndex = Math.max(
    0,
    projection.scenarios.findIndex((scenario) => scenario.name === "Base"),
  );
...
function setActiveScenario(index, projection, { highlightRow = true } = {}) {
  ...
  elements.scenarioFocusCase.textContent = `${scenario.name} - ${formatUsdCompact.format(scenario.fdv)} FDV`;
...
function render() {
  ...
  elements.projectionNote.textContent =
    `Moonsheet assumes ${formatUsdCompact.format(SNAPSHOT_TVL_USD)} TVL at Season 1 snapshot. FDV scenarios run from $150M to $1B in $50M steps; airdrop pool is ${formatPercent.format(AIRDROP_PERCENT * 100)}% of FDV. Projection assumes linear point pace from ${projectionStartLabel} to ${snapshotLabel}; public APIs expose cumulative points, not a guaranteed future emission schedule.`;
...
function buildCsv() {
  const header = ["wallet_label", "address", "points", "confirmed", "pending", "share_percent", "rank"];
